...
function AddBannerComponent({ navigate, form, categories, errors, onInputChange, onImageChange, onSubmit, preview }) {
    return (
                ...
                <div className="mb-1">
                    ...
                    <div className="flex items-center gap-2">
                        <Switch
                            ...
                            className={`${
                                form.flag ? 'bg-green-500' : 'bg-gray-200'
                            } relative inline-flex h-6 w-11 items-center rounded-full transition-colors`}
                        >
                            <span className="sr-only">Toggle Status</span>
                            <span
                                className={`${
                                    form.flag ? 'translate-x-6' : 'translate-x-1'
                                } inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
                            />
                        </Switch>
                        <span className="text-gray-700 text-lg">{form.flag ? "Active" : "Inactive"}</span>
                    </div>
